"""
Ticket 6 — Evaluation & Confusion Matrix Report (ATA classifier).

Runs the trained model against the TEST split (never used for training or
checkpoint selection -- that was val) and produces:
  - overall accuracy / macro-F1 (comparable to the PRD's success metric)
  - per-class precision/recall/F1 (exposes which classes are actually weak,
    which a single headline number hides)
  - a confusion matrix image
  - a metrics JSON, for the Model Performance Dashboard (Ticket 16) to read later
"""
import json
...
MODEL_DIR = "models/ata_classifier"
TEST_CSV = "data/processed/test.csv"
MAX_LENGTH = 256
OUT_DIR = "reports/ata_classifier"
...
def main():
    ...
    print(f"Test examples (ATA-labeled): {len(df)}")

    true_labels = df["ata_chapter_label"].astype(str).tolist()
    ...
    print(f"\n=== TEST SET RESULTS (never used in training or checkpoint selection) ===")
    print(f"Accuracy: {acc:.1%}   (PRD target: >=80%)")
    ...
    ax.set_ylabel("True")
    ax.set_title("ATA Chapter Classifier — Confusion Matrix (Test Set)")
